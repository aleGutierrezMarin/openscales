package org.openscales.fx.control.layer
{
	...
	{		
		...
		public function setUp():void
		{
			...
			_map.map.addLayer(_layer1);
			_map.map.addLayer(_layer2);
			_map.map.addLayer(_layer3);
		...
		{
			var current:uint = _map.map.layerContainer.getChildIndex(_layer2);
			_position.upLayer(new MouseEvent(MouseEvent.MOUSE_DOWN));			
			Assert.assertEquals(current+1, _map.map.layerContainer.getChildIndex(_layer2));
		}
